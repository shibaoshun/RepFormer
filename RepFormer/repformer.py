...
class WindowAttention(nn.Module):
    def __init__(self, dim, win_size, num_heads, token_projection='linear', qkv_bias=True, qk_scale=None, attn_drop=0.,
        ...
        # prompt
        self.F_ext_net = F_ext(in_nc=1, nf=256)
        self.prompt_scale0 = nn.Linear(256, 256, bias=True)
        self.prompt_shift0 = nn.Linear(256, 256, bias=True)

        self.CA1 = CALayer1(dim, reduction=4, bias=False)
    def forward(self, x, xm,  img_size=(128, 128), attn_kv=None, mask=None):
        B_, N, C = x.shape
        H = img_size[0]
        W = img_size[1]
        B = int(B_ / (H * W / int(self.win_size[0]) / int(self.win_size[0])))
        ...
        if mask is not None:
            ...
            attn = attn.view(-1, self.win_size[0], self.win_size[0], C)
            attn = attn.permute(0, 3, 1, 2)
            w = self.CA1(attn)
            attn = (attn * w).permute(0, 2, 3, 1)
            attn = attn.reshape(-1, self.num_heads, self.win_size[0] * self.win_size[0],C // self.num_heads, )  # (128,4,4,256)
            #############################################
        ...
        ##################################
        prompt = self.F_ext_net(xm)
        scale = self.prompt_scale0(prompt)
        shift = self.prompt_shift0(prompt)
        ##################################

        x = (attn @ v).transpose(1, 2).reshape(B_, N, C)

        # merge windows
        x = x.view(-1, self.win_size[0], self.win_size[0], C)  # (128,4,4,256)
        x = window_reverse(x, self.win_size[0], H, W)  # B H' W' C(2,32,32,256)
        x = x.view(B, H * W, C)
        x = x * scale.view(-1,1,C) + shift.view(-1,1,C) + x

        x = rearrange(x, ' b (h w) (c) -> b h w c ', h=H, w=W)
        x = window_partition(x, self.win_size[0])  # nW*B, win_size, win_size, C  N*C->C #
        x = x.reshape(B_, N, C)
...
def window_reverse(windows, win_size, H, W, dilation_rate=1):
    # B' ,Wh ,Ww ,C
    B = int(windows.shape[0] / (H * W / int(win_size) / int(win_size)))
...
class CATransformerBlock(nn.Module):
    ...
    def forward(self, x, xm, xmb,mask=None, img_size=(128, 128)):
        B, L, C = x.shape
        H = img_size[0]
        W = img_size[1]
        assert L == W * H, \
...
class SIMTransformerBlock(nn.Module):
    ...
    def forward(self, x, xm, xmb,mask=None, img_size=(128, 128)):
        ...
        # partition windows
        x_windows = window_partition(shifted_x, self.win_size)  # nW*B, win_size, win_size, C  N*C->C # (64,8,8,128)
        x_windows = x_windows.view(-1, self.win_size * self.win_size, C)  # nW*B, win_size*win_size, C

        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, xm, img_size, mask=attn_mask)  # nW*B, win_size*win_size, C

        # merge windows
        attn_windows = attn_windows.view(-1, self.win_size, self.win_size, C)  # (128,4,4,256)

        shifted_x = window_reverse(attn_windows, self.win_size, H, W)  # B H' W' C(2,32,32,256)
...
class RepFormer(nn.Module):
    ...
    def forward(self, x, xma, xmb, mask=None):
        ...
        # Encoder
        conv0 = self.encoderlayer_0(y, xm,xmb, mask=mask, img_size=self.img_size)
        pool0 = self.dowsample_0(conv0, img_size=self.img_size)

        self.img_size = (int(self.img_size[0] / 2), int(self.img_size[1] / 2))
        conv1 = self.encoderlayer_1(pool0, xm,xmb, mask=mask, img_size=self.img_size)
        pool1 = self.dowsample_1(conv1, img_size=self.img_size)  # (1,4096,128)

        self.img_size = (int(self.img_size[0] / 2), int(self.img_size[1] / 2))
        conv2 = self.encoderlayer_2(pool1, xm, xmb,mask=mask, img_size=self.img_size)
        pool2 = self.dowsample_2(conv2, img_size=self.img_size)
        self.img_size = (int(self.img_size[0] / 2), int(self.img_size[1] / 2))

        # Bottleneck
        conv3 = self.conv(pool2, xm,xmb, mask=mask, img_size=self.img_size)

        # Decoder
        up0 = self.upsample_0(conv3, img_size=self.img_size)
        self.img_size = (int(self.img_size[0] * 2), int(self.img_size[1] * 2))
        deconv0 = torch.cat([up0, conv2], -1)
        deconv0 = self.decoderlayer_0(deconv0, xm, xmb,mask=mask, img_size=self.img_size)

        up1 = self.upsample_1(deconv0, img_size=self.img_size)
        self.img_size = (int(self.img_size[0] * 2), int(self.img_size[1] * 2))
        deconv1 = torch.cat([up1, conv1], -1)
        deconv1 = self.decoderlayer_1(deconv1, xm, xmb,mask=mask, img_size=self.img_size)

        up2 = self.upsample_2(deconv1, img_size=self.img_size)
        self.img_size = (int(self.img_size[0] * 2), int(self.img_size[1] * 2))
        deconv2 = torch.cat([up2, conv0], -1)
        deconv2 = self.decoderlayer_2(deconv2, xm, xmb,mask=mask, img_size=self.img_size)
