...
if opt.resume:
    ...
    print('------------------------------------------------------------------------------')
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, opt.nepoch-start_epoch+1, eta_min=1e-6,last_epoch=217)
...
train_dataset = get_training_data(opt.train_dir, img_options_train, opt.aug_times)
...
for epoch in range(start_epoch, opt.nepoch + 1):
    ...
    for i, data in enumerate(train_loader, 0): 
        ...
        mask2 = data[3].cuda()
        if epoch > 5:
            target, input_, mask, mask2 = utils.MixUp_AUG().aug(target, input_, mask, mask2)
        with torch.cuda.amp.autocast():
            restored = model_restoration(input_, mask, mask2)
        ...
        if (index+1)%eval_now==0 and i>0:
            ...
            with torch.no_grad():
                ...
                print("[Ep %d it %d\t PSNR1 : %.4f] " % (epoch, i, psnr_val_rgb))
                with open(logname,'a') as f:
