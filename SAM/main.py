...
def test_visualize(args, model):
    ...
    for fname in tqdm(fnames[:num_visualize]):
        ...
        masks_pred_sam_prompted2, _, _ = predictor.predict(
            ...
            box=bbox[None, :],
            multimask_output=False,)
        # plt.imsave('1.png', masks_pred_sam_prompted2.squeeze(), cmap="gray")
        ...
        # plot the results
        fig0, ax1 = plt.subplots(1, 1, figsize=(15, 10))
        ax1.set_title('Box')
        box_one(bbox, ax1, masks_pred_sam_prompted2.squeeze(), i)
        ...
        ax[3].set_title('Box')
        # box_one(bbox, ax[3], masks_pred_sam_prompted2.squeeze())
        show_box(bbox, ax[3])
...
def test(args):
    ...
    for train_index, text_index in kf.split(images):
        ...
        for idx in range(len(test_images)):
            ...
            img_emb = get_embedding(image, predictor = predictor)
            img_emb = img_emb.cpu().numpy().transpose((2, 3, 1, 0)).reshape((64, 64, 256)).reshape(-1, 256)
...
def main():
    ...
    parser.add_argument('--k', type=int, default=6, help='number of pics')
    ...
    parser.add_argument('--visualize_num', type=int, default=10, help='number of pics to visualize')
